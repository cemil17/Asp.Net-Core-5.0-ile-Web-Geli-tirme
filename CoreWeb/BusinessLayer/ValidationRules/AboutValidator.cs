...
namespace BusinessLayer.ValidationRules
{
	public class AboutValidator : AbstractValidator<About>
	{
		public AboutValidator()
		{
			...
			RuleFor(x => x.Age).NotEmpty().WithMessage("Bu alan boş geçilemez.");
			RuleFor(x => x.Age).MaximumLength(2).WithMessage("Geçerli bir yaş giriniz.");

			RuleFor(x => x.Mail).NotEmpty().WithMessage("Bu alan boş geçilemez.");
			RuleFor(x => x.Mail).EmailAddress().WithMessage("Lütfen geçerli mail adresini girin.");
			RuleFor(x => x.Mail).MinimumLength(5).WithMessage("Minimum 5 (beş) karakter girmelisiniz.");
			RuleFor(x => x.Mail).MaximumLength(100).WithMessage("Maksimum 100 (yüz) karakter girmelisiniz.");
